...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var characters:Array<String> = /*CoolUtil.coolTextFile('assets/data/characterList.txt')*/ CoolUtil.textFileLines('characterList');
	...
	function loadSong(daSong:String):Void
	{
		...
		FlxG.sound.playMusic(Paths.playableSong(daSong), 0.6);

		// WONT WORK FOR TUTORIAL OR TEST SONG!!! REDO LATER
		vocals = new FlxSound().loadEmbedded(Paths.playableSong(daSong, true));
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.B){
			var fullNoteLiat:Array<Array<Dynamic>> = [];

			for(sec in _song.notes)
				for(nt in sec.sectionNotes){
					fullNoteLiat.push(nt);
					sec.sectionNotes.remove(nt);
				}

			fullNoteLiat.sort((A,B) -> Std.int(A[0] - B[0]));

			for(nt in fullNoteLiat){
				trace(nt[0] / Conductor.stepCrochet);
				nt[0] = Math.round(nt[0] / Conductor.stepCrochet);
				nt[2] = Math.floor(nt[2] / Conductor.stepCrochet);
				var fsec:SwagSection = _song.notes[Math.floor(nt[0] / 16)];

				nt[3] = fsec.mustHitSection ? 1 : 0;
				//nt[3] = (nt[1] >= 4  ? 1 : 0);
				nt[1] =  nt[1] % 4;
				//trace('ST: ${nt[0]} ND: ${nt[1]} MP: ${nt[3]}');
				fsec.sectionNotes.push(nt);
			}
	...
	{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase(), 0);
		FlxG.resetState();
	}
