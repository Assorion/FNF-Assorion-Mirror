...
class OptionsState extends MusicBeatState
{
	...
	override public function keyHit(ev:KeyboardEvent){
		...
		if(t != -1){
			...
			switch(optionSub[curSub][curSel]){
				case 'start_volume':
					Settings.pr.start_volume = CoolUtil.boundTo(Settings.pr.start_volume + ch, 0, 100, true);
				...
				// visuals
				case 'framerate':
					Settings.pr.framerate = CoolUtil.boundTo(Settings.pr.framerate + ch, 0, 500, true);
		...
		if(key.hardCheck(NewControls.UI_BACK)){
			...
			Settings.apply();
			Settings.flush();
			FlxG.sound.play(Paths.lSound('cancelMenu'));
	...
	{
		FlxG.sound.play(Paths.lSound('scrollMenu'), 0.4);
