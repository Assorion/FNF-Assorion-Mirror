...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		*/
		...
		gf.antialiasing = Settings.pr.antialiasing;
	...
	public inline function handleStage(){
		...
		switch(curStage){
			case 'stage', '':
				defaultCamZoom = 0.9;
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.lImage('stageback'));
					bg.antialiasing = Settings.pr.antialiasing;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.lImage('stagefront'));
				...
				var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.lImage('stagecurtains'));
	...
	private inline function introSpriteTween(spr:FlxSprite){
		spr.alpha = 1;
	...
	function startCountdown():Void
	{
		...
		var startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			FlxG.sound.play(Paths.lSound(introAssets[swagCounter + 4]), 0.6);
			if(introSprites[swagCounter] != null)
				introSpriteTween(introSprites[swagCounter]);
		...
		for(fNote in section.sectionNotes){
			...
			if(susLength > 1)
				for(i in 0...susLength){
					var susNote = new Note(time + i + 2, noteData, true, i == susLength-1);
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.x = healthBar.x + ((1 - (health * 0.01)) * healthBar.width) - 26;
		iconP2.x = iconP1.x;

		health = healthBar.percent = CoolUtil.boundTo(health, 0, 100);
		...
		//{
			...
			/if (camFollow.x != dad.getMidpoint().x + 150 && !mustHitSection)
			{
			...
			if (mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
		...
		notes.forEachAlive(function(daNote:Note){
			var dir = Settings.pr.downscroll ? 22 : -22;
			...
			if((daNote.player != playerPos || Settings.pr.botplay) && songTime >= daNote.strumTime){
				...
				if(Settings.pr.light_bot_strums){
					...
					postEvent(Conductor.stepCrochet * 1.1 * 0.001, () -> {
						playAnimAndCenter(strumRef, 'static');
					});
				}
	...
	{
		health -= Settings.pr.miss_health * 0.5;
		if (combo > 5)
		{
			gf.playAnim('sad');
		}
